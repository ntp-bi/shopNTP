...
import com.ntp.be.dto.OrderItemDetail;
import com.ntp.be.dto.OrderRequest;
import com.ntp.be.entities.*;
import com.ntp.be.repositories.OrderRepository;
...
public class OrderService {
    ...
    public OrderResponse createOrder(OrderRequest orderRequest, Principal principal) throws Exception {
        ...
        Address address = user.getAddressList().stream()
                .filter(address1 -> orderRequest.getAddressId().equals(address1.getId())).findFirst().orElseThrow(BadRequestException::new);
    ...
    public List<OrderDetails> getOrdersByUser(String name) {
        ...
        return orders.stream().map(order -> {
            return OrderDetails.builder()
                    ...
                    .orderItemDetails(getItemDetails(order.getOrderItemList()))
                    .exceptedDeliveryDate(order.getExpectedDeliveryDate())
                    .build();
        }).toList();
    }
